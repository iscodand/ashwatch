...
# The command API (C#) publishes the Log domain entity with default
# System.Text.Json casing (PascalCase). Normalize to camelCase here so the
# storage shape is consistent regardless of the producer's language/casing.
_FIELD_MAP = {
    "Id": "id",
    "TenantId": "tenantId",
    "ProjectId": "projectId",
...
def _normalize(data: dict) -> dict:
    normalized = {_FIELD_MAP.get(key, key): value for key, value in data.items()}
...
def lambda_handler(event, context):
    ...
    for record in event["Records"]:
        ...
        try:
            data = _normalize(_parse_record_body(record["body"]))
